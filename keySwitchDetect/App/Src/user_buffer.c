 ...
 ****************************************************************************************
 */
volatile static uint8_t ringBuffer[MAX_BUFFER_SIZE]; 
/**
 ****************************************************************************************
...
void user_buffer_create(RingBuffer* buffer, int buffer_size, int low_watermark, int high_watermark)
{
    ...
    buffer->buffer_size = buffer_size;//优化丢包
    buffer->readIdx     = 0;
    buffer->writeIdx    = 0;
    buffer->endIdx      = buffer_size;
    
    //memory allocation of circular buffer
//    buffer->data_ptr = (uint8_t*)malloc((long)(buffer->buffer_size)*sizeof(uint8_t)); 
		 buffer->data_ptr = (uint8_t*)ringBuffer; 
    ...
    if(high_watermark < buffer_size && low_watermark <= high_watermark && low_watermark > 0){
...
{
    if(buffer != NULL && buffer->data_ptr != NULL){
        return true;
    }
    return false;
}
...
/**
 ***************************************************************************************
 * @brief function used to write item into ring buffer
 * 
 * @param buffer        buffer to push data
 * @param wrdata        pointer with data which should be pushed
 * @param write_amount  amount of bytes which should be pushed to buffer
 ***************************************************************************************
 */
...
uint16_t user_buffer_read_items(RingBuffer* buffer, uint8_t* rddata, uint16_t read_amount)
{
    ...
    readIdx += read_amount;
    if(readIdx >= endIdx + 1 && readIdx > writeIdx){
        //endIdx has a proper value only when writeIdx is lower than readIdx
        readIdx -= (endIdx + 1);
    }
...
void user_buffer_release_items(RingBuffer* buffer, uint16_t read_amount)
{
	...
	if(buffer->readIdx >= buffer->endIdx + 1 && buffer->readIdx > buffer->writeIdx){
        //endIdx has a proper value only when writeIdx is lower than readIdx
...
{
	user_buffer_release_items(buffer, user_buffer_item_count(buffer));
}
